...
@Component({
  selector: 'app-perfil-cosas-lindas',
  templateUrl: './perfil-cosas-lindas.page.html',
  styleUrls: ['./perfil-cosas-lindas.page.scss'],
})
export class PerfilCosasLindasPage implements OnInit {
  ...
  toPerfil() {
    this.router.navigate(['perfil-cosas-lindas']);
  }
  ...
  async subirFoto() {
    const options : CameraOptions = {
      ...
      await this.camera.getPicture(options).then((imageData) => {
       ...
       const subirString = storage().ref(`cosas-lindas/${this.perfil}_${this.getFecha()}`);
  ...
  traerFotos(queTraer)
  {
    ...
    const imagenes = storage().ref("cosas-lindas"); //referencia a carpeta
